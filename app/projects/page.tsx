      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {projectStrengths.map((item) => (
            <article key={item.title} className={`min-h-52 rounded-[2rem] border border-dashboard-outline-variant p-5 shadow-subtle ${item.tone}`}>
              <h3 className="text-2xl font-black tracking-[-0.05em]">{item.title}</h3>
              <p className="mt-3 text-sm font-semibold leading-6 text-dashboard-on-surface-variant">{item.text}</p>
            </article>
          ))}
        </div>
      </section>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_0.8fr] lg:items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href={`/projects/${featuredProject.slug}`} className={`rounded-full bg-ghost-white px-5 py-3 text-sm font-black text-pitch-black ${focusRing}`}>Featured case</Link>
              <Link href="/contact" className={`rounded-full border border-ghost-white/25 px-5 py-3 text-sm font-black text-ghost-white ${focusRing}`}>Contact</Link>
            </div>
          </div>
          <div className="relative min-h-64 rounded-[2rem] bg-gradient-to-br from-clay-violet via-ube-haze to-pitch-black">
            <div className="absolute inset-6 rounded-[1.5rem] border border-ghost-white/15" />
            <div className="absolute bottom-6 left-6 right-6 rounded-2xl bg-ghost-white/10 p-4 backdrop-blur-sm">
              <p className="text-sm font-black">Portfolio OS</p>
              <p className="mt-1 text-xs font-semibold text-ghost-white/70">Proof · Personality · Interaction</p>
            </div>
          </div>
        </div>
      </section>
