    ...
    >
      <div className={`mb-7 flex gap-2 ${isCollapsed ? "flex-col items-center" : "items-center justify-between"}`}>
        ...
        <button
          ...
          className={`grid size-9 shrink-0 place-items-center rounded-xl border border-dashboard-outline-variant bg-dashboard-surface-low text-sm font-black transition-all hover:-translate-y-0.5 hover:border-dashboard-outline ${focusRing}`}
        >
          {isCollapsed ? ">" : "<"}
        </button>
      </div>
      ...
      <nav className="space-y-1" aria-label="Primary navigation">
        {navItems.map((item) => (
          <Link
            ...
            className={`group flex items-center rounded-xl px-3 py-2.5 text-sm font-semibold transition-all hover:-translate-y-0.5 ${focusRing} ${
          ...
          >
            ...
            {!isCollapsed && item.active ? <span className="h-5 w-1 rounded-full bg-lime-pop" /> : null}
          </Link>
        ))}
      </nav>
