...
export function MobileSidebar({ navItems, focusRing }: MobileSidebarProps) {
  ...
  useEffect(() => {
    ...
    const idleId = "requestIdleCallback" in window ? window.requestIdleCallback(prefetchRoutes) : globalThis.setTimeout(prefetchRoutes, 1200);
  ...
  return (
    <>
      ...
      <div 
        ...
        role="dialog" 
        aria-modal="true" 
        aria-labelledby={`${sidebarId}-title`}
      >
        <button
          type="button"
          aria-label="Tutup menu"
          className="absolute inset-0 bg-pitch-black/30 backdrop-blur-[2px]"
          onClick={() => setIsOpen(false)}
        />
        <aside
          ref={panelRef}
          id={sidebarId}
          className={`absolute right-0 top-0 flex h-dvh w-[min(84vw,300px)] flex-col overflow-y-auto border-l border-dashboard-outline-variant bg-dashboard-surface-lowest p-4 shadow-subtle transition-transform duration-300 ease-in-out ${
            isOpen ? "translate-x-0" : "translate-x-full"
          }`}
        >
          <div className="flex items-center justify-between gap-3">
            <Link href="/" onClick={() => setIsOpen(false)} className={`flex items-center gap-3 rounded-2xl ${focusRing}`}>
              <span className="grid size-9 place-items-center rounded-xl bg-pitch-black text-sm font-bold text-ghost-white dark:bg-ghost-white dark:text-pitch-black">
                R
              </span>
              <span>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center justify-between rounded-xl px-3 py-2 text-sm font-semibold ${focusRing} ${
                    active
                      ? "bg-pitch-black text-ghost-white dark:bg-ghost-white dark:text-pitch-black"
