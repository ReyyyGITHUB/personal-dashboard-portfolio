              ...
              <div>
                ...
                <h1 className="max-w-2xl text-4xl font-bold leading-[1.04] tracking-[-0.04em] text-dashboard-on-surface sm:text-5xl lg:text-5xl">
                  ...
                  <span className="lg:hidden" aria-hidden="true">
                    Gua bikin <span className="text-clay-violet underline decoration-lime-pop decoration-[0.12em] underline-offset-[0.16em]">cerita proyek yang bisa dicek</span>.
                  </span>
                ...
                <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                  <Link
                    href="/projects"
                    className={`inline-flex items-center justify-center gap-2 rounded-xl bg-pitch-black px-5 py-3 text-sm font-bold text-ghost-white transition-transform hover:-translate-y-0.5 dark:bg-ghost-white dark:text-pitch-black ${focusRing}`}
                  >
                    Lihat proyek
            ...
            <section className="lg:col-span-8 min-w-0">
              ...
              <div className="relative">
                <div className="grid gap-4 pb-20 md:grid-cols-2">
                  {featuredProjects.map((project) => (
                    ...
                    <div className="pointer-events-none mt-auto -mx-4 -mb-4 flex min-h-24 items-end bg-gradient-to-t from-ghost-white via-ghost-white/90 to-transparent p-4 dark:from-dashboard-surface-lowest dark:via-dashboard-surface-lowest/90">
                      <Link
                        href="/projects"
                        className={`pointer-events-auto inline-flex items-center gap-2 rounded-xl bg-pitch-black px-4 py-2.5 text-sm font-bold text-ghost-white underline-offset-4 transition-colors hover:underline dark:bg-ghost-white dark:text-pitch-black ${focusRing}`}
                      >
                        Lihat selengkapnya
                        <BrandIcon name="arrow-up-right" className="size-5" />
                      </Link>
                ...
                <div className="pointer-events-none absolute inset-x-0 bottom-0 flex h-72 items-end justify-center rounded-b-[2rem] bg-gradient-to-t from-ghost-white via-ghost-white/92 via-60% to-ghost-white/0 pb-24 dark:from-dashboard-background dark:via-dashboard-background/90 dark:to-dashboard-background/0">
                  <div className="flex w-full items-center gap-4 px-6 justify-center">
                    <span className="hidden h-px flex-1 bg-gradient-to-r from-transparent via-lime-pop to-lime-pop/60 sm:block" />
                    <Link
                      href="/projects"
                    ...
                    <span className="hidden h-px flex-1 bg-gradient-to-l from-transparent via-lime-pop to-lime-pop/60 sm:block" />
                  </div>
                </div>
              </div>
            </section>
              ...
              <section className="rounded-3xl border border-dashboard-outline-variant bg-dashboard-surface-lowest p-5 shadow-subtle transition-transform hover:-translate-y-0.5">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {stack.length > 8 && (
                    <Link
                      href="/stack"
                      className="rounded-full border border-dashboard-outline-variant bg-dashboard-surface-lowest px-2.5 py-1 text-xs font-bold text-dashboard-on-surface-variant hover:border-dashboard-outline hover:bg-dashboard-surface-low hover:text-dashboard-on-surface transition-all"
                    >
                      +{stack.length - 8} lainnya
                    </Link>
                  )}
                </div>
              </section>
...
function GitHubWidgetsFallback() {
  return (
    <>
      <article className="rounded-3xl border border-dashboard-outline-variant bg-dashboard-surface-low p-5 shadow-subtle">
        <div className="h-9 w-36 rounded-xl bg-dashboard-outline-variant/60" />
        <div className="mt-5 grid grid-flow-col grid-rows-7 gap-1">
          {Array.from({ length: 91 }).map((_, index) => (
            <span key={index} className="size-2.5 rounded-[3px] bg-dashboard-outline-variant/70" />
          ))}
        </div>
        <div className="mt-4 h-4 w-44 rounded bg-dashboard-outline-variant/60" />
      </article>
      <article className="rounded-3xl border border-dashboard-outline-variant bg-dashboard-surface-low p-5 shadow-subtle">
        <div className="h-9 w-32 rounded-xl bg-dashboard-outline-variant/60" />
        <div className="mt-4 space-y-3">
          {[0, 1, 2].map((item) => (
            <div key={item} className="rounded-2xl border border-dashboard-outline-variant bg-dashboard-surface-lowest p-3">
              <div className="h-4 w-4/5 rounded bg-dashboard-outline-variant/60" />
              <div className="mt-2 h-3 w-1/2 rounded bg-dashboard-outline-variant/50" />
            </div>
          ))}
        </div>
      </article>
